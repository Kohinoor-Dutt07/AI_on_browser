...
def list_available_credentials() -> Dict[str, str]:
    """
    List all available credentials from environment.
    Returns dict of site -> username (decrypted)
    """
    try:
        secure_manager = SecureCredentialManager()
        return secure_manager.list_available_credentials()
    except Exception:
...
def set_credential(site: str, username: str, password: str):
    """
    Programmatically set a credential (for runtime, not persistent).
    """
    try:
        secure_manager = SecureCredentialManager()
        secure_manager.set_credential(site, username, password)
    except Exception:
        # Fallback to original implementation
        env_key = f"CREDENTIALS_{site.upper().replace('.', '_').replace('-', '_')}"
